...
Upgrade_MySQL2MariaDB()
{
    ...
    case "${mariadb_version}" in
        ...
        *) Echo_Red "Supported migration targets are MariaDB 10.6, 10.11, 11.4, 11.8 and 12.3."; exit 1 ;;
    esac
...
EOF
    ...
    if ! /usr/local/mariadb/bin/mysql --defaults-file="${LNMP_MYCNF}" < "/root/mysql_all_backup${Upgrade_Date}.sql"; then
